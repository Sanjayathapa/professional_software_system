...
class UpdategroupScreen extends StatefulWidget {
  ...
  UpdategroupScreen({required this.id});
  @override
  _UpdateEnrollmentScreenState createState() => _UpdateEnrollmentScreenState();
}

class _UpdateEnrollmentScreenState extends State<UpdategroupScreen> {
  final _formKey = GlobalKey<FormState>();
  final TextEditingController groupNameController = TextEditingController();
  @override
  Widget build(BuildContext context) {
    final posDropdownProvider =
                      ...
                      DropDownWidget(
                        ...
                        onChanged: (value) {
                          if (value != null && value.isNotEmpty) {
                            ...
                            print(
                                'Selected Course ID: ${posDropdownProvider.selectedCourseId}');
                          }
                        },
                          ...
                          if (posDropdownProvider.selectedCourseId != null) {
                            try {
                             final groupName = groupNameController.text.trim();
                              await groupProvider.updategroup(widget.id,
                                  posDropdownProvider.selectedCourseId!,groupName);
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(' Successful Updated Group',
                                      style: TextStyle(color: Colors.white)),
                                  backgroundColor:
                                      const Color.fromARGB(255, 1, 131, 9),
                                ),
                              );
                            } catch (e) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text('Failed to Update group',
                                      style: TextStyle(color: Colors.white)),
                                  backgroundColor:
                                      const Color.fromARGB(255, 251, 6, 6),
                                ),
                              );
                            }
                          }
